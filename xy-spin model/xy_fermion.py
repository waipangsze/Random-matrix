...
def Hamiltonian(n, gamma, disorder):
    ...
    for __ in range(set_ensemble):
        ...
        w = w/n
        wnegetive = w[:n] # take negative energy only
...
def Hamiltonian_part_spectrum(n, gamma, disorder):
    """
    we set a center energy and take energy spectrum around it may set as 100
    """
    if n >= 12:
        set_ensemble = 500
    else:
    ...
    for se in range(set_ensemble):
        ...
        w = np.linalg.eigvalsh(H)
        w = w/n
        wneq = w[:n] # take negative energy only
